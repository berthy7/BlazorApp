...
namespace BlazorApp.Client.Services
{
    public class AuthDataServices : IAuthDataServices
    {
        ...
        public AuthDataServices(HttpClient httpClient, IOptions<AppSettings> options)
        {
            ...
            httpClient.BaseAddress = new Uri(appSettings.UrlSeguridad);
        ...
        public async Task<User> ObtenerUserPorTokenAsync(string autToken)
        {
            try
            {
                ...
                {
                    loginResult.user = new User()
                    {
                        userid = int.Parse(claims.FirstOrDefault(c => c.Type == nameof(User.userid)).Value),
                        username = claims.FirstOrDefault(c => c.Type == nameof(User.username)).Value,
                        matricula = claims.FirstOrDefault(c => c.Type == nameof(User.matricula)).Value,
                        nombre = claims.FirstOrDefault(c => c.Type == nameof(User.nombre)).Value,
                        correo = claims.FirstOrDefault(c => c.Type == "email").Value,
                        role = claims.FirstOrDefault(c => c.Type == nameof(User.role)).Value
                    };
                }
